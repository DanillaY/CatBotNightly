...
class Radio_Client(commands.Cog):
    ...
    async def on_ready(self) -> None:
        await print_message_async('Radio cog started working')
    
    @commands.command()
    async def radio(self,ctx, radio_id) -> None:
        if ctx.author.voice != None and len(self.bot.get_cog('Youtube_Client').youtube_queue) == 0:
            await print_message_async('Started playing radio')
    ...
    async def radio_search_by_country(self,ctx) -> None:
        try:
            ...
            if len(radios) == 0:
                ...
                await print_message_async('Bot is searching radios by country')
                message = join_radio_info(get_radios_by_country(country))
                await ctx.send('```'+message+'```')
        except BaseException as e:
            await print_message_async('Error while getting coutries from database', str(e))
    ...
    async def radio_random(self,ctx) -> None:
        try:
            ...
            await print_message_async('Error while starting new radio', str(e))
...
async def play_radio(self:Radio_Client,ctx,radio_id) -> None:
    ...
    try:
        ...
        await print_message_async('Could not stream that radio', str(e))
